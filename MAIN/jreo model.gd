...
func _ready():
	contact_point = Position3D.new()
	add_child(contact_point)


	
func _physics_process(delta):
	cast_to.y = -(rest_length + wheel_size)*0.5
	
	var velocity: Vector3 = (global_translation - past_position)/delta
	var local_velocity: Vector3 = global_transform.basis.orthonormalized().xform_inv(velocity)
	past_position = global_translation
	...
	if is_colliding():
		...
		contact_point.global_translation = coll_point
		...
		var contact_velocity: Vector3 = contact_point.global_transform.basis.orthonormalized().xform_inv(velocity)

		var compressed: float = abs(cast_to.y) -coll_point.distance_to(global_translation)
		...
		car.apply_impulse(contact_point.global_translation-car.global_translation,forces)
	else:
		contact_point.translation = cast_to
	
	$debug.translation = contact_point.translation +Vector3(0,wheel_size*0.5,0)
